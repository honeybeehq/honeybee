...
The separate [Mini A/A snapshot run](evidence/h10-mini/mini-h10-aa-snapshot-v4.json) also completed four independent children at 100,000 deliveries. The source-aligned analyzer finds one consumed Map and a 3,670,056-byte shallow backing table in every stopped snapshot, then zero HSR drivers in every released snapshot. All eight classifications and input/output digests pass the [parent proof](evidence/h10-mini/mini-h10-aa-snapshot-v4-proof.json). [Compressed originals](evidence/h10-mini/mini-h10-aa-snapshot-v4-archive.json) preserve the structures. Snapshot-mode numeric readings remain diagnostic-only and are not compared with the numeric table above.
