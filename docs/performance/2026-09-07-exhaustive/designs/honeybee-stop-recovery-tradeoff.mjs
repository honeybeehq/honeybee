...
try{
 for(const scenario of scenarios){
  ...
  seed.transact(()=>{seed.createBee({id:'target',name:'target',handle:'SR.target',agent:'stub',substrate:'hsr',cwd:dir});for(let gen=1;gen<=scenario.gens;gen++){if(gen>1)assert.equal(seed.reviveBee('target').generation,gen);seed.updateRuntimeState('target',gen,'stopped',{exitCause:'clean'});}seed.createBee({id:'writer',name:'writer',handle:'SR.writer',agent:'stub',substrate:'hsr',cwd:dir});seed.updateRuntimeState('writer',1,'stopped',{exitCause:'clean'});});
  close(seed);
  ...
  const insert=db.prepare(`INSERT INTO commands(verb,bee_id,args,target_generation,status,attempts,next_attempt_at,enqueued_at,finished_at,failure_cause,idempotency_key) VALUES(?,'target',?,?,'done',1,0,0,1,NULL,NULL)`);
  ...
  for(const [name,fn] of Object.entries({predicate:r=>r.store.hasStopThenReviveRequest('target',scenario.probe),boot:r=>r.core.boot()})){
  ...
  for(const r of rigs){const revived=r.store.reviveBee('writer');r.store.updateRuntimeState('writer',revived.generation,'running');}
  const writes={};
  for(const terminal of ['done','failed']){
   const write=r=>{const ids=[];for(let j=0;j<100;j++){const cmd=r.store.enqueueCommand('stop','writer',{thenRevive:false});const claimed=r.store.claimNextCommand();ids.push([cmd.id,claimed?.id]);if(terminal==='done')r.store.completeCommand(cmd.id);else r.store.reportCommandFailure(cmd.id,'node_unreachable');}return ids;};
  ...
  assert.equal(state(rigs[0].store),state(rigs[1].store));
  for(const r of rigs){const history=r.store.listCommands({beeId:'target'});assert.equal(history.length,scenario.count);assert(history.every(c=>c.status==='done'&&c.args.thenRevive===false));const written=r.store.listCommands({beeId:'writer'});assert.equal(written.length,1400);assert.equal(written.filter(c=>c.status==='done').length,700);assert.equal(written.filter(c=>c.status==='failed').length,700);close(r.store);}
