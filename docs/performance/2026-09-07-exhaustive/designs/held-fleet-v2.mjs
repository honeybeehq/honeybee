...
const [before, after, out, changes, roundsArg = '15', countsArg = '1,1000,10000', blockedArg='false', perBeeArg='1', urgencyArg='idle'] = process.argv.slice(2);
assert(before && after && out && changes !== undefined);
assert(['false','true'].includes(blockedArg)); const blocked=blockedArg==='true',perBee=Number(perBeeArg);assert(Number.isSafeInteger(perBee)&&perBee>=1&&perBee<=10000);assert(['idle','next'].includes(urgencyArg));
...
const report = { completed: false, startedAt: new Date().toISOString(), sources, changed, toolHash, sqlToolHash, workload:{blocked,perBee,urgency:urgencyArg},
  environment: { node: process.version, execArgv: process.execArgv, cpu: cpus()[0].model, boot: boot(), loadBefore: loadavg() },
  scope: 'Real CoreStore and DaemonCore with FakeDriver. Real-running Bees with configurable held messages; optional stopping Cell moves keep the delivery fence engaged. Whole uninstrumented step CPU/wall; separate one-step SQL diagnostic after timing. Exact pending/state/audit/driver invariants. No real process, delivery or end-to-end latency claim. Copied fixtures, distinct modules, ABBA, setup/checks/GC outside timing.', results: [] };
...
try {
  for (const count of countsArg.split(',').map(Number)) for (const i1Enabled of [false,true]) {
    assert(Number.isSafeInteger(count) && count>=1 && count<=10000 && count*perBee<=100000);
    const seedPath = join(dir, `seed-${count}-${i1Enabled}.sqlite`), seed = modules[0].openCoreStore(seedPath, { now: () => 1000 });
    seed.transact(() => {
      for (let i=0;i<count;i++) {
        const id=`pending-${i}`;
        seed.createBee({id,name:id,agent:'stub',substrate:blocked?'cell':'hsr',cwd:dir});
        seed.updateRuntimeState(id,1,'running');
        assert.equal(seed.currentRuntime(id).bootEvidence,'real');
        for(let j=0;j<perBee;j++)seed.send(id,'x'.repeat(64),{urgency:urgencyArg});
        if(blocked){
          const cell=seed.putCell({sourceBeeId:id,originRepo:'/tmp/origin',sha:'abc',wrapper:id,spaceName:'held-space-c1',spaceDir:dir,gitCommonDirRealpath:'/tmp/origin/.git',objectFormat:'sha1'});
          const move=seed.admitBeeMove({beeId:id,idempotencyKey:id,requestHash:id,expected:{placementVersion:0,cellId:cell.id},destinationCwd:'/tmp/destination'});
          assert.equal(move.phase,'stopping');
        }
      }
    });
    seed.close();
    ...
    for (const rig of rigs) {
      ...
      assert.equal(rig.store.listUndeliveredMessages().length, count*perBee);
