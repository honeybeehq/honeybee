...
const indexName='mailbox_pending_metadata',indexSql=`CREATE INDEX ${indexName} ON mailbox(bee_id,id,urgency,enqueued_at,delivered_at) WHERE delivered_at IS NULL`;
...
const report={completed:false,startedAt:new Date().toISOString(),sources,toolHash,sqlToolHash,control,indexSql,environment:{node:process.version,cpu:cpus()[0].model,boot:boot(),loadBefore:loadavg()},workload:{scenarios,rounds,order:'ABBA',warmups:3,writeBatch:100},scope:'Two immutable production roots with distinct module instances and exact schema/store source delta. Initially byte-identical pre-index databases; public constructor installs the index. ABBA fresh-file opens include first index installation with file copying, assertions and close outside clock. Held mail seeded by public API, delivered history inserted offline with real bee ID and foreign-key checks. Real CoreStore/DaemonCore reads and durable public send+cancel, send+delivery and send+expedite+cancel writes. FakeDriver; no workers or live runtime. Shared process heap/cache can cross sides. Allocation and native memory not measured. Storage measured on closed checkpointed files.',results:[]};
