...
const [rootArg,outArg,roundArg='15']=process.argv.slice(2), root=resolve(rootArg), rounds=Number(roundArg);
assert(Number.isInteger(rounds)&&rounds>=3&&rounds<=30);
...
for(const spec of [
 ...
 try{
  ...
  const db=new DatabaseSync(paths[1]);const install=measure(()=>db.exec(index));db.close();delete install.value;
