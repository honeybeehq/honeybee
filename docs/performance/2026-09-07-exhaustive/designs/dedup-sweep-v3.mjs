...
function treeHashes(dir) {
  ...
  const walk = (rel) => {
    for (const entry of readdirSync(join(dir, rel), { withFileTypes: true })) {
      ...
      else if (next.includes('/src/') || next === 'v2/daemon/tests/helpers.ts' || next.endsWith('package.json') || next === 'package-lock.json') {
...
function extractSide(name, ref) {
  ...
  const archive = execFileSync('git', ['-C', repo, 'archive', sha, 'v2', 'package.json', 'package-lock.json'], { maxBuffer: 512 * 1024 * 1024 });
...
const report = {
  schemaVersion: 3, completed: false, startedAt, timestamp: null,
  ...
  repo, sides: sides.map((s) => ({ ref: s.ref, sha: s.sha,
    ...
    treeFileCount: Object.keys(s.hashes).length, hashes:s.hashes })),
  changedSourceFiles, moduleIdentityShared,
  workload: { pending, rounds, cadence: CADENCE, mode, warmupCyclesPerSide:2,
...
try {
  assert.ok(report.environment.bootIdentity, 'a verifiable macOS boot identity is required');
  for (const scenario of ['wide-cadence', 'probe-cadence']) {
    ...
    for (const side of [0,1,1,0]) { const ctx=prepareCycle(side,fixture,scenario); runTicks(ctx); verifyCycle(ctx); }
    const raw = [
  ...
  assert.deepEqual(bootIdentity(),report.environment.bootIdentity,'OS boot changed during the run');
