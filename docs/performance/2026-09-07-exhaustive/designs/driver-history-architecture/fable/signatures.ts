...
export declare class HsrDriverSketch {
   ...
   * values, and write ordering keep compatible observable outcomes and
   * effect order in BOTH arms (the guard is the only new instruction;
   * source and built bytes necessarily change). The sites remain the test evidence origin.
   */
...
// (driver-tmux/src/driver.ts:365 echo-mismatch assume-best, :384 normal
// verified submission) take the same in-place guards; the daemon's tmux block adds
...
export interface DeliveryHistoryRecorder {
   ...
   * write sites run no user-supplied callback and permit no reentry — the
   * relevant distinction versus arbitrary recorder code — so a throwing
   * recorder would create a NEW mid-acceptance exception path (the analog
   * of the Tmux now() edge); a
