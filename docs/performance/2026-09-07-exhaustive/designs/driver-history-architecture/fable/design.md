...
- **Design 2 — test-owned recorder sink, default absent** (fully sketched
  ...
  interface whose no-throw/no-reentry contract is prose (today's write
  sites run no user-supplied callback and permit no reentry — the relevant
  distinction versus arbitrary recorder code; a throwing recorder is a NEW
  mid-acceptance exception path, and the Tmux record-before-now() edge
  proves after-the-fact recording is not exactly equivalent). Design 1 reaches the same daemon end state and
...
2. Disabled matrix: the SAME transition matrix with
   ...
   pending/confirmed contents, stop/revive — observably equal to the default run
   in outcomes and effect order, except both queries throw with the
   stable message.
