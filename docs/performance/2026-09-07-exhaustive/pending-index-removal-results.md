...
Mini candidate build, all v2 typechecks, 225 core tests and 364 daemon/CLI tests passed, with one platform skip. Tests cover reinstalling the replacement on old schema, repeated old-index recreation and drop, unchanged schema version, selective full-body access, FIFO and the empty-mail guard. Parent review found the original migration test captured expected rows before creating the old index and compared only IDs for the global list. Tests-only `39631b5f`, integrated as `7bf65e1b`, now compares raw old-plus-covering reads with new-only reads using two interleaved Bees and full rows. The before plan must use the old index; the after plan must use the replacement. Public projections preserve full values and explicit group/FIFO order. Parent follow-up `5eb66fad` validates order-key types without coercion. The affected suite, core typecheck and repository build pass on Mini. The immutable tradeoff ruler already compares the distinct production sides; no performance rerun is needed for that test correction.
