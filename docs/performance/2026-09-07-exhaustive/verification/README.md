...
Combined Cell-move/main integration is frozen as tree7d7e6287cb9de1e7dbd5b5914b882b213894a2c9 in mini-cellmove-perf-frozen.json. Parent commits recreate that exact tree before the test-only readiness repair7eb8ecb7. Build and all v2 checks pass; core212, serial daemon299/one skip, CLI59, adapters70. Full Cell initially has four immediate-delivery assertions fail; identical failures reproduce on6e4447d4 without the Cell optimization. The bounded not_ready-only test helper then passes full Cell70/one skip, driver-cell typecheck, and build. Raw failures, baseline reproduction, and final logs are retained.
