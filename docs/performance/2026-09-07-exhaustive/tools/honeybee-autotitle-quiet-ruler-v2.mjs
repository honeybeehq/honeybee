...
try {
  for (const scenario of SCENARIOS) {
    ...
    seed.close();
    // Offline rows carry retained delivered HISTORY plus a pending tail so
    // both mailbox partial-index arms are exercised: every row except each
    // third is delivered by the bee's real generation 1 (created above).
    ...
    report.scenarios.push({
      ...
      offlinePendingRows,
      auditRowsAdded: 0, // offline inserts write no audit rows and quiet scans none — both asserted above
      pkProofRows,
