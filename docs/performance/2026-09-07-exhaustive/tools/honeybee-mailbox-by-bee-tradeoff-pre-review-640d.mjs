...
const report={completed:false,startedAt:new Date().toISOString(),sources,toolHash,sqlToolHash,control,indexName,
 environment:{node:process.version,cpu:cpus()[0].model,boot:boot(),loadBefore:loadavg()},
 workload:{scenarios,rounds,order:'ABBA',warmups:3,writeBatch:100,deliveredFraction:'every third seeded row is pending unless allTargetPending; extraDelivered rows are all delivered'},
 scope:'Immutable modules; exactly schema.ts and store.ts differ; public constructor installs index on byte-identical pre-index databases. Offline mailbox fixture uses real existing UUID Bees and foreign-key checks, but does not synthesize enqueue audit/projection history. Only listMessages and pending/full-body authority reads consume those rows. Public timed send/cancel, send/deliver, and send/expedite/cancel writes retain production WAL/NORMAL durability and normal semantic audit. Read clocks exclude validation and diagnostics. Same-process caches/GC may cross sides. No retained-memory measurement. Open includes installation but excludes copy, close and inspection; caches warm. Storage on checkpointed closed files.',results:[]};
...
try{
 for(const scenario of scenarios){
  ...
  const fixtureSha256=hash(readFileSync(seedPath)),openRaw=[[],[]];
  for(let n=0;n<3;n++)for(const side of [0,1,1,0]){
   ...
   const info=storage(path);assert.equal(info.index.length,side===1&&!control?1:0);assert.equal(info.quickCheck,'ok');rmSync(path);
  }
  ...
  const beforeWritesStorage=paths.map(storage),stores=paths.map((path,side)=>open(side,path));opened.push(...stores);
  ...
  report.results.push({scenario,fixtureSha256,openRaw,openMetrics:summarize(openRaw),beforeWritesStorage,results,diagnostics,queryPlans,writes,afterWritesStorage});save();
 }
