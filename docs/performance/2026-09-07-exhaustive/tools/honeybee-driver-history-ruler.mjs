...
async function child(spec){
  ...
  assert.equal(toolSha256,spec.toolSha256);
  ...
  const ownershipPath=out+'.owned.json';
  assert(!existsSync(ownershipPath));
  const ownership=(state,host=null)=>writeFileSync(ownershipPath,JSON.stringify({state,dir,host,root,count,toolSha256,hostIdentity},null,2)+'\n');
  // A hard child timeout bypasses finally. Persist the owned fixture and exact
  // process identity so the parent can inspect it before any cleanup signal.
  ownership('fixture_created');
  ...
  assert.deepEqual(Reflect.get(driver,'hostLaunch'),{kind:'source',entry:join(root,'v2/driver-hsr/src/runner-host-main.ts')});
  ...
  function liveState(){
    ...
    const socket=Reflect.get(p,'socket');
    if(socket===null)return null; // Parsed boot can precede the socket-connect retry.
    assert.equal(typeof socket,'object');
    if(Reflect.get(socket,'destroyed')||!Reflect.get(socket,'writable'))return null;
  ...
  try{
    ...
    ownership('host_started',host);
    await until(()=>{drain();return observations.some(e=>e.kind==='booted'&&!e.synthetic);},'boot witness');
    await until(()=>{drain();return liveState()!==null;},'booted and connected write lane');
    await memory('booted');
    let retries=0;
    ...
    await until(()=>{drain();const r=liveState();return r!==null&&Object.entries(r).every(([k,v])=>k==='legacySharedObservation'?v===false:v===0);},'drained protocol and socket');
    const protocolAtRead=liveState();assert.notEqual(protocolAtRead,null);
    const wire=wireProof();history();drain();
    ...
    const base={beeId:'history-probe',generation:1};
    assert.deepEqual(normalized,[{...base,kind:'booted'},{...base,kind:'turn_ended'},
      ...(count>0?[{...base,kind:'turn_started',synthetic:true}]:[]),{...base,kind:'exited',exitCause:'stopped_by_system'}]);
    assert.deepEqual(evidence,[{...base,flag:'spawn_failed',action:'clear',detail:'runtime booted'}]);
    assert.deepEqual(sessions,[{...base,sessionId:'history-probe'}]);
    assert.deepEqual(cursors,[{...base,cursor:Buffer.byteLength(ready)}]);
    ...
    ownership('fixture_removed',host);
  }
}
  ...
  try{
    for(const count of counts){
      ...
      for(const [orderIndex,side] of result.order.entries()){
        ...
        const spec={root:roots[side],count,out:childOut,mode,disabled:comparison==='candidate'&&side===1,source:sources[side],hostIdentity,toolSha256};
