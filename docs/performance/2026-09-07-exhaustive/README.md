...
[Cell-move discovery](cell-move-discovery-results.md) measures the new active-pointer ordering against 100,000 retained receipts. [Standing-mail dedup](retained-standing-candidate.md) records the accepted Unit 2 change, its reentry regression and fix, retained heap benefit, and growth-sweep CPU cost. The two Cell read changes passed combined verification and are accepted; Unit 2 has passed its final allocation/probe and combined verification gates. Fresh `mini-main7eb-*` timing reports and separate profiles establish the current-main baseline after Cell-move support.
