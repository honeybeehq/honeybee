...
- Undo: a clean eviction loses nothing that git holds; the bee revives into the same path at the same commit. What is not recoverable: ignored files other than preserved `.env` files (caches and `.tmp`), which is why dirty and forced evictions are explicit operator acts and are logged as `forced=true`.
