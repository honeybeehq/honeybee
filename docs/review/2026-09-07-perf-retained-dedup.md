...
No blocking correctness finding. The measured tradeoff is roughly 1–2 microseconds with nonempty retained sets. Thirty drained cohorts remove all 30,000 obsolete I1 IDs and reduce final post-GC heap by 646,304 bytes in both repeats. RSS is higher in those captures, so acceptance as an RSS optimization is explicitly unsupported. At 500000 messages the repeat still removes every obsolete ID and lowers final retained JS heap by about10.48MB, but RSS is 2–20MB higher. This unexplained resident cost stays open alongside standing-backlog retention and rotated-generation bookkeeping; acceptance is specifically removal of unbounded obsolete bookkeeping.
