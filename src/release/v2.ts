/** Role-bound v2 metadata. Parsing establishes consistency, not authenticity.
 * Version-specific validation is intentionally separate from the frozen v1 reader.
 */
...
const schema = JSON.parse(readFileSync(new URL("../../contracts/release/v2/schema.json", import.meta.url), "utf8"));
...
export type ArtifactRole = "caller" | "provider" | "honeybee";
/** Caller is evidence context, not necessarily a destination installation target. */
export type Combination = {
  caller: ComponentIdentity & { component: "apiary" | "apiaryd" };
  provider: ComponentIdentity & { component: "apiaryd" };
...
export type ContractFingerprint = { role: ArtifactRole; component: Component; fingerprint: string };
...
export function parseCompatibilityRecord(value: unknown): CompatibilityRecord {
  ...
  for (const operation of subject.operations) {
    requireContract(operation.provider.role !== operation.consumer.role, "provider and consumer roles must differ");
    for (const endpoint of [operation.consumer, operation.provider]) {
      requireContract(subject.combination[endpoint.role].component === endpoint.component, "endpoint role/component mismatch");
    }
...
export function parseCompatibilityMatrix(value: unknown): CompatibilityMatrix {
  ...
  for (const entry of matrix.entries) {
    ...
    for (const identity of Object.values(entry.subject.combination)) {
...
export function parseReleaseManifest(value: unknown): ReleaseManifest {
  ...
  for (const protocol of manifest.dependencyLock.requiredProtocols) {
    const operations = pinned.subject.operations.filter((operation) => operation.required
      && operation.provider.role === "honeybee" && operation.protocol.name === protocol.name
