...
async function cmdAction(ctx: CliContext, parsed: Parsed): Promise<number> {
  const usage = [
    ...
    "       hive action cancel <actionId> [--force]   (queued or failed actions; --force for delivered/in-flight attempts) | action retry <actionId> [--force]",
    "       hive action complete <actionId> [--output k=v]... [--detail d]   (operator: mark a running, waiting-on-input or failed agent action succeeded; commit fills commitSha from the checkout HEAD)",
