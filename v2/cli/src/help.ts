...
const GROUPS: Group[] = [
  {
    ...
    rows: [
      ...
      ["fork", "<bee> [--name n]", "copy a conversation; no instruction or requested compaction"],
      ["thread-fork", "<bee> [--name n]", "durable conversation copy with a pinned history boundary (local Codex HSR)"],
      ["thread-handoff", "<bee> --instruction <text>", "fork, compact with the instruction, then continue; preserves the source"],
      ["thread-operation", "get|retry <operation-id>", "read progress or recover a failed thread operation on its existing successor"],
  ...
  },
  {
    ...
    rows: [
      ...
      ["action", "report <id> --attempt n --token t …", "(from inside a bee) report an action's outcome: --succeeded [--output k=v] | --failed | --ask q | --progress n"],
      ["action", "enqueue <bee> <kind> [--input k=v]", "append to the bee's action queue (commit, land, archive, …) · list · get · definitions"],
      ["action", "cancel|retry <id> · pause|resume <bee>", "queue controls (cancel pending or failed, retry failed, pause releases) · complete <id> · reorder <bee> <id>…"],
