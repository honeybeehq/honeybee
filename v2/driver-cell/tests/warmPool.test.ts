...
const BUILD = { racyIndexWindowMs: 0 } as const;
...
test("provisionCell replays (no re-clone) after a claim — idempotency survives the pool", () => {
  const { cellsRoot, origin, cleanup } = setup();
  try {
    buildPoolMember(cellsRoot, { originRepo: origin.repo, repoName: "origin", sha: origin.sha }, OPTS, BUILD);
...
test("claim applies a checkout delta to a reachable older sha", () => {
  ...
  try {
    ...
    buildPoolMember(cellsRoot, { originRepo: origin.repo, repoName: "origin", sha: sha2 }, OPTS, BUILD);
...
test("a member behind the origin fetches the wanted commit and serves it", () => {
  const { cellsRoot, origin, cleanup } = setup();
  try {
    buildPoolMember(cellsRoot, { originRepo: origin.repo, repoName: "origin", sha: origin.sha }, OPTS, BUILD);
    const sha2 = commitOn(origin.repo, "src/added.ts", "export const two = 2;\n", "second");
    const req = reserve(cellsRoot, origin, "bee-behind", sha2);
    const claimed = claimFromPool(cellsRoot, req);
    assert.ok(claimed, "a member behind the origin should catch up by fetch");
    const paths = cellPaths(cellsRoot, req.wrapper, req.repoName, req.cellId);
    assert.equal(g(paths.spaceDir, ["rev-parse", "HEAD"]), sha2);
    assert.equal(g(paths.spaceDir, ["status", "--porcelain"]), "");
    assert.ok(existsSync(join(paths.spaceDir, "src", "added.ts")));
  } finally {
    cleanup();
  }
});

test("a sha the origin lacks is not served, and the member is kept", () => {
  const { cellsRoot, origin, cleanup } = setup();
  try {
    buildPoolMember(cellsRoot, { originRepo: origin.repo, repoName: "origin", sha: origin.sha }, OPTS, BUILD);
    const req = reserve(cellsRoot, origin, "bee-unknown", "0".repeat(40));
    assert.equal(claimFromPool(cellsRoot, req), null, "an unreachable sha must not be served from the pool");
    assert.equal(poolMemberCount(cellsRoot, repoKeyFor(origin.repo, "origin")), 1, "the member stays usable");
  } finally {
    cleanup();
  }
});
...
test("buildPoolMember honours maxSize", () => {
  ...
  try {
    ...
    assert.ok(buildPoolMember(cellsRoot, req, OPTS, { ...BUILD, maxSize: 1 }));
    assert.equal(buildPoolMember(cellsRoot, req, OPTS, { ...BUILD, maxSize: 1 }), null, "must not exceed maxSize");
    assert.equal(poolMemberCount(cellsRoot, repoKeyFor(origin.repo, "origin")), 1);
  } finally {
    cleanup();
  }
