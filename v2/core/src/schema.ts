 ...
 *  v21 — Cell→checkout move: `bees.placement_version`, `bees.active_move_id`,
...
CREATE TABLE IF NOT EXISTS bees (
  ...
  handle           TEXT,
  -- v21: Cell→checkout placement generation (CAS for bee.move).
  placement_version INTEGER NOT NULL DEFAULT 0,
  -- v21: in-flight bee_moves.id; NULL when idle (complete/failed receipts remain).
  active_move_id   TEXT,
  -- v21: cells.id for the active or retained Cell.
  cell_id          TEXT
) STRICT;
...
/**
 * v21 — at most one in-flight move pointer per bee. Created after additive
