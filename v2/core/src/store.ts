...
import { BEES_ADDITIVE_COLUMNS, FLAGS_ADDITIVE_COLUMNS, HANDLE_INDEX_SQL, IDEMPOTENCY_INDEX_SQL, MAILBOX_ADDITIVE_COLUMNS, MAIL_HISTORY_INDEX_SQL, MAIL_HISTORY_PROJECTION_SQL, RUNTIMES_ADDITIVE_COLUMNS, SCHEMA_SQL, SCHEMA_VERSION } from "./schema.ts";
import {
  LOGIN_FLOW_PHASES,
...
export class CoreStore {
  ...
  mailHistory(params: MailHistoryParams = {}): MailHistoryResult {
    ...
    const deletedByBee = new Map<string, AuditRow | null>();
    for (const message of candidates) {
      ...
      if (message.lifecycle.state === "queued") {
        ...
        if (deleted !== null && deleted.seq > message.seq) {
          message.lifecycle = {
            ...
            reason: "bee_deleted",
            canceledAt:
              deleted.payload.deletedAt === undefined
                ? deleted.ts
                : auditNumber(deleted.payload.deletedAt, `bee.deleted deletedAt at seq ${deleted.seq}`),
          };
        }
      }
    }
  ...
  private latestBeeDeletedRow(beeId: string, snapshotSeq: number): AuditRow | null {
    const row = this.stmt(
      `SELECT * FROM audit
