...
Retained receipt: baselines/account-activity-reads.json. Three rounds each: empty0→0, 1bee1→0, 120bees180→0, 2000bees3000→0. Every public result hash matches the baseline. The empty/control cases do not establish overall admission cost; currentRuntime still runs per bee per candidate account and reservation/roster scans remain. The Node test exit status is authoritative for behavioral controls; receipt completeness describes count cases only.
